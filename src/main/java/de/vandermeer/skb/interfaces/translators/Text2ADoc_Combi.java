 ...
 * @since      v0.0.3
 */
public interface Text2ADoc_Combi extends CombinedTranslator {

	/**
	 * Returns a new combined translator for text to ASCIIDOC.
	 * @return new combined translator
	 */
	static Text2ADoc_Combi create(){
		return new Text2ADoc_Combi() {
		};
	}

	@Override
	default String text2tmp(String input) {
		return Text2ADoc_HEntity.create().text2tmp(input);
	}

	@Override
	default String tmp2target(String input) {
		return Text2ADoc_HEntity.create().tmp2target(input);
	}
	...
	default String translateCharacters(String input) {
		return Text2ADoc_Character.create().translateCharacters(input);
	}

	@Override
	default String translateHtmlElements(String input) {
		return Text2ADoc_HEntity.create().translateHtmlElements(input);
	}
