 ...
 * @since      v0.0.3
 */
public interface Text2ADoc_HEntity extends HtmlElementTranslator {

	/**
	 * Creates a new text to ASCIIDOC HTML-element translator.
	 * @return new translator
	 */
	static Text2ADoc_HEntity create(){
		return new Text2ADoc_HEntity() {
		};
	}
	...
	static final String[] REPLACEMENT_LIST_HE = new String[]{
		"(((/abbr)))", "(((/b)))", "(((/code)))", "(((/i)))", "(((/li)))", "(((/ol)))", "(((/sub)))", "(((/sup)))", "(((/ul)))", "(((abbr)))", "(((b)))", "(((br /)))", "(((br/)))", "(((br)))", "(((code)))", "(((i)))", "(((li)))", "(((ol)))", "(((sub)))", "(((sup)))", "(((ul)))"
	};

	/** Array of AsciiDoc replacements for temporary HTML Element strings. */
	static final String[] AD_LIST_HE = new String[]{
		"", "*", "+", "_", "", "", "", "", "", "", "*", "\n", "\n", "\n", "+", "_", "* ", "", "_", "^", ""
	};

	@Override
	default String text2tmp(String input){
		return StringUtils.replaceEach(input, SEARCH_LIST_HE, REPLACEMENT_LIST_HE);
	}

	@Override
	default String tmp2target(String input){
		return StringUtils.replaceEach(input, REPLACEMENT_LIST_HE, AD_LIST_HE);
	}

	@Override
	default String translateHtmlElements(String input){
		return StringUtils.replaceEach(input, SEARCH_LIST_HE, AD_LIST_HE);
	}
}
