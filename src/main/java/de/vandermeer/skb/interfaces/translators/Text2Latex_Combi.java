 ...
 * @since      v0.0.3
 */
public interface Text2Latex_Combi extends CombinedTranslator {

	/**
	 * Returns a new combined translator for text to LaTeX.
	 * @return new combined translator
	 */
	static Text2Latex_Combi create(){
		return new Text2Latex_Combi() {
		};
	}

	@Override
	default String text2tmp(String input) {
		return Text2Latex_HEntity.create().text2tmp(input);
	}

	@Override
	default String tmp2target(String input) {
		return Text2Latex_HEntity.create().tmp2target(input);
	}
	...
	default String translateCharacters(String input) {
		return Text2Latex_Character.create().translateCharacters(input);
	}

	@Override
	default String translateHtmlElements(String input) {
		return Text2Latex_HEntity.create().translateHtmlElements(input);
	}
